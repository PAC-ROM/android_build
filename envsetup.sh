...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^pac_") ; then
       PAC_BUILD=$(echo -n $1 | sed -e 's/^pac_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $PAC_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       PAC_BUILD=
    fi
    export PAC_BUILD
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the PAC model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch pac_$target-$variant
        fi
    fi
    return $?
}
...
function eat()
{
    ...
    if (adb shell getprop ro.pac.device | grep -q "$PAC_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $PAC_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.pac.device | grep -q "$PAC_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function pacgerrit() {
    ...
    local user=`git config --get review.review.pac-rom.com.username`
    if [ -z "$user" ]
    then
        read -p "Please provide your PAC Gerrit username: " user
    fi
    local review=`git config --get remote.pac.review`
    if [ -z "$review" ]
    then
        local review=`git config --get remote.ss.review`
    fi
    local project=`git config --get remote.pac.projectname`
    if [ -z "$project" ]
    then
        local project=`git config --get remote.ss.projectname`
    fi
    local command=$1
    shift
    case $command in
